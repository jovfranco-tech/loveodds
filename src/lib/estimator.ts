...
export function loComputeFunnel(c: Criteria): { steps: FunnelStep[]; pct: number; finalN: number; base: number } {
  ...
  if (c.edadMin != null && c.edadMax != null) {
    ...
    // adult age spectrum is 18-65 (47 years)
  ...
  if (c.hijos === 'Sin hijos') {
    const r = 0.42; // aprox 42% de adultos en rango fertil no tienen hijos o no viven con ellos
    cur = cur * r;
    steps.push({
  ...
  if (c.nacionalidad) {
    // Colombians, Venezuelans, Spaniards in MX constitute a very select pool
    // Colombia in CDMX represents ~0.4%, Argentina ~0.2%, USA ~0.8%, etc.
    let r = 0.004;
  ...
  if (c.estaturaMin != null && c.estaturaMin > 0) {
    let r = 1.0;
    if (c.busca === 'Mujer') {
      const z = (c.estaturaMin * 100 - 157) / 6; // mean=157cm, sd=6cm
      r = Math.max(0.001, 1 - normCdf(z));
    } else if (c.busca === 'Hombre') {
      const z = (c.estaturaMin * 100 - 169) / 7; // mean=169cm, sd=7cm
      r = Math.max(0.001, 1 - normCdf(z));
    } else {
      const z = (c.estaturaMin * 100 - 163) / 8; // mean=163cm, sd=8cm (combined)
      r = Math.max(0.001, 1 - normCdf(z));
    }
    cur = cur * r;
    steps.push({
  ...
  if (c.escolaridad) {
    ...
    if (c.escolaridad === 'Licenciatura+') {
      r = 0.34; // 34% of active workforce has professional degree
    } else if (c.escolaridad === 'Posgrado+') {
      r = 0.06; // 6% master/doctorate degree
...
export function loScenarios(c: Criteria): Scenario[] {
  ...
  // Scenario 1: Expand age spectrum if narrow
  ...
  // Scenario 2: Search in ZMVM instead of only CDMX
  if (c.ubicacion === "CDMX") {
    addScenario(
  ...
  // Scenario 3: Decrease minimum income requirement
  ...
  // Scenario 4: Accept Divorced individuals
  if (c.estado === "Soltero" || c.estado === "Soltera") {
  ...
  // Scenario 5: Remove nationality barrier
  if (c.nacionalidad) {
    addScenario(
  ...
  // Scenario 6: Flexibilize minimum height requirement
...
export function estimatePopulation(criteria: Criteria): EstimationResult {
  const { steps, pct, finalN, base } = loComputeFunnel(criteria);
  ...
  const realityCheck = generateRealityCheck(criteria, pct, finalN, mostRestrictiveFilter);
  const whatIfSuggestions = loScenarios(criteria);
